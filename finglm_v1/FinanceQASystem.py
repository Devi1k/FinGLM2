...
class FinanceQASystem:
    def __init__(self):
        ...
        df1 = pd.read_excel(self.data_dictionary_path, sheet_name='库表关系')
        df1['库表名中文'] = df1['库名中文'] + '.' + df1['表中文']
        df1['库表名英文'] = df1['库名英文'] + '.' + df1['表英文']
        self.database_table_ch = list(df1['库表名中文'])
        self.database_table_en = list(df1['库表名英文'])
        df1['representation'] = '库表名：' + df1['库表名中文'] + '，注释：' + df1['表描述']
        self.database_table_map = df1.set_index('库表名中文')['库表名英文'].to_dict()

        self.llm = LLMClient()  # LLM API客户端
        self.dialogue_manager = DialogueManager()  # 对话管理
        self.vector_store = TableVectorStore(list(df1['representation']))  # 内存向量存储
    ...
    def process_question(self, question_id: str, question: str) -> str:
        ...
        # 2. 理解问题
        understanding = self.parser_agent.parse(question, context,self.database_table_ch)
